...
void Player::initialize(glm::vec2 position, float speed, int width, sbe::Colour colour, GLuint textureID, sbe::InputManager* inputManager) {
	position_ = position;
	speed_ = speed;
	width_ = width;
	colour_ = colour;
...
void Player::update(const std::vector<std::string>& levelData, int tileWidth) {
	if (inputManager_->isKeyDown(SDLK_w)) position_.y += speed_;
	if (inputManager_->isKeyDown(SDLK_a)) position_.x -= speed_;
	if (inputManager_->isKeyDown(SDLK_s)) position_.y -= speed_;
	if (inputManager_->isKeyDown(SDLK_d)) position_.x += speed_;

	collideWithLevel(levelData,  tileWidth);
}

void Player::draw(sbe::SpriteBatch& spriteBatch) {
	glm::vec4 destRect(position_.x, position_.y, width_, width_);
	spriteBatch.draw(destRect, textureID_, colour_);
}

bool Player::collideWithLevel(const std::vector<std::string>& levelData, int tileWidth) {
...
void Player::checkTilePosition(const std::vector<std::string>& levelData, std::vector<glm::vec2>& collideTilePositions, float x, float y) {
